// Define the roles that are above Pylon in the array below
...
discord.interactions.commands.register(
  ...
  async (interaction, { member, nickname }) => {
    ...
    if (
      blacklistedRoles.includes(member.roles[0]) ||
      member.user.id === guild.ownerId
    )
      return await interaction.respondEphemeral(
        ':x: `Permission Error` Cannot change nickname of this member.'
      );
